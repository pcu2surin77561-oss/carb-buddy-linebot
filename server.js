...
(async () => {
    try {
        ...
        aiQueue = new PQueue({ 
            concurrency: 1,
            intervalCap: 12, // จำกัด 12 requests เพื่อไม่ให้เกินโควตา 15 ครั้ง/นาที (Free Tier)
            interval: 60000  // รีเซ็ตทุกๆ 1 นาที (60,000 ms)
        });
        logger.info("✅ โหลดระบบ AI Queue (Concurrency: 1, RateLimit: 12/min) สำเร็จ");
    } catch (err) {
...
async function discoverGeminiModels() {
    ...
    try {
        ...
        // ✅ ทดสอบทีละตัว (สำคัญมาก)
        for (const modelName of candidateModels) {
            try {
                logger.info(`🧪 Testing: ${modelName}`);
                ...
                if (is429) {
                    logger.warn(`⚠️ 429 แต่เก็บไว้: ${modelName}`);
                    working.push(modelName);

                    // 🔥 หน่วงหนักขึ้น
                    await new Promise(r => setTimeout(r, 5000));
                } else {
                    logger.warn(`❌ ใช้ไม่ได้: ${modelName}`);
                }
            }
        }
            ...
            try {
                ...
                if (isRateLimit) {
                    if (attempts < maxAttempts) {
                        logger.warn(`⚠️ Gemini ติด Rate Limit (429), รอ 10 วินาทีเพื่อลองใหม่ (ครั้งที่ ${attempts}/${maxAttempts})...`);
                        await new Promise(r => setTimeout(r, 10000));
                        continue; // ลองใหม่โมเดลเดิม
                    } else {
                        logger.warn(`🚨 Rate Limit (429) โควตา API Key เต็ม ยกเลิกการหาโมเดลสำรอง`);
                        throw new Error("ขณะนี้มีผู้ใช้งานพร้อมกันจำนวนมาก (คิว AI เต็ม) กรุณาลองใหม่อีกครั้งใน 1-2 นาทีครับ 🙏");
                    }
                } else if (isNotFound) {
