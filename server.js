...
(async () => {
    try {
        ...
        logger.info("✅ โหลดระบบ AI Queue (Concurrency: 1) สำเร็จ");
    } catch (err) {
            ...
            try {
                ...
                if (isRateLimit) {
                    if (attempts < maxAttempts) {
                        logger.warn(`⚠️ Gemini ติด Rate Limit (429) ใน ${modelName}, รอ 30 วินาทีแล้วลองใหม่ (ครั้งที่ ${attempts}/${maxAttempts})...`);
